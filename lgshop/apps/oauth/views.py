...
class QQoauthView(View):
    ...
    def get(self,request):
        ...
        return JsonResponse({"code":"1","error_message":"","login_url":login_url})
...
class QQAuthUserView(View):
    def get(self,request):
        ...
        try:
            ...
            return render(request,"oauth_callback.html",{"error_message":"该用户未绑定"},context=context)
        else:
            #已经绑定
            #状态保持
    ...
    def post(self,request):
        ...
        if sms_code_redis is None:
            return render(request,"oauth_callback.html",{"error_message":"短信验证码已经失效了"})
        if sms_code != sms_code_redis:
            return render(request,"oauth_callback.html",{"error_message":"短信验证码输入错误"})
        #判断openid是否存在
        openid = check_openid(access_token_openid)
        if openid is None:
            return render(request,"oauth_callback.html",{"error_message":"openid已经失效"})
        #查询用户是否已经存在
        try:
            user = Users.objects.get(mobile=mobile)
        except Exception as e:
            #如果没有这个对象 就创建一个对象
            user = Users.objects.create_user(mobile=mobile,username=mobile,password=password)
        else:
            #用户存在就检查密码是否正确
            if not user.check_password(password):
                return render(request,"oauth_callback.html",{"error_message":"密码与原密码不一致"})
        #已经存在这个对象 就进行openid和user绑定
        try:
            QQauth_user = QQauthUser.objects.create(openid=openid,user=user)
        except Exception as e:
            return render(request,"oauth_callback.html",{"error_message":"密码输入错误"})
        else:
            #状态保持
            login(request,user)
            #跳转到之前界面
